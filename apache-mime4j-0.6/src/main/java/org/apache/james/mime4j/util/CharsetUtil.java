...
public class CharsetUtil {
    ...
    static {
        ...
        boolean hasBig5 = false;
        boolean hasHKSCS = false;

        for (Charset c : JAVA_CHARSETS) {
            try {
                // force BIG5_HKSCS to be "decodingSupported"
                if (!c.canonical.equals("Big5_HKSCS"))
                    new String(dummy, c.canonical);
                else if (!hasBig5 && hasHKSCS)
                    new String(dummy, c.canonical);
        ...
        for (Charset c : JAVA_CHARSETS) {
            if (c.canonical.equalsIgnoreCase("big5_hkscs") && !hasHKSCS)
                continue;
            ...
            // Map HKSCS to BIG5 if no hkscs but has big5 on system
            if (c.canonical.equalsIgnoreCase("big5") && !hasHKSCS && hasBig5) {
                charsetMap.put("big5_hkscs", c);
                charsetMap.put("big5-hkscs", c);
                charsetMap.put("big5hkscs", c);
            }
        }
     ...
     * @param bytes
     *            byte array to test.
     * @return <code>true</code> if the specified byte array consists entirely of
     *         US ASCII characters, <code>false</code> otherwise.
     */
